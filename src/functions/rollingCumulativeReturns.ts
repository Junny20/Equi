function rollingReturns(data: number[], days: number): number[] {
  ...
  let rollingCumulativeReturns: number[] = [];

  for (let i = 0; i < data.length; i++) {
    if (!i) {
      rollingCumulativeReturns.push(data[i]);
    } else if (i <= days) {
      ...
      rollingCumulativeReturns.push(cumulativeReturn - 1);
    } else {
  ...
  return rollingCumulativeReturns;
}
